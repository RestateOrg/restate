...
class _OrderRequestState extends State<OrderRequest> {
  ...
  Future<void> updatestock(int index) async {
    ...
    final snapshot1 = await firestore
        ...
        .where('machinery_name', isEqualTo: product['machinery_name'])
        .get();
    final item = snapshot1.docs[0];
    final stock = int.parse(item['available quantity']);
    ...
    await firestore
        ...
        .doc(useremail)
        .collection('inventory')
        .doc(product['machinery_name'])
        .update({'available quantity': "$newstock"});
    final snapshot2 = await firestore
        .collection('machinery inventory')
        .where('image_urls', isEqualTo: imageurls)
        .get();
